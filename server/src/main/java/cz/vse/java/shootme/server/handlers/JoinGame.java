...
public class JoinGame {
    ...
    public JoinGame(JoinGameRequest request) {
        Game game = Server.get().getGame("default");

        request.getConnection().setGame(game);
        game.getConnections().add(request.getConnection());

        game.getState().getEntities().add(new Player("img/players/player_1.png", new Vector(0, 0), new Vector(1, 1), request.getConnection().getUser().getUsername()));
